...
#' data(Zhangetal_cpgs)
#' data(Illig_data)
#' data(dummySamplesheet)
#' data(CS_final_coefs, FS_final_coefs, NS_final_coefs)
#' result <- epismoker(dataset = dummyBetaData, samplesheet = dummySamplesheet, ref.Zhang = Zhangetal_cpgs, ref.Elliott =  Illig_data, 
...
#' @export
#' 
MLM <- function(dataset, samplesheet, ref.CS, ref.FS, ref.NS){
  message("============================================")
  ...
  res_MLM <- setNames(data.frame(cbind( rownames(coeffs),coeffs,getSmokingStatus(coeffs[,c(1:3)]),coeffs_probs,getSmokingStatus2(coeffs_probs[,c(1:3)]),
                                        as.character(samplesheet$SmokingStatus))),
                      c("SampleName","logCoeff_CS","logCoeff_NS","logCoeff_FS","PredictedSmokingStatus","probs_CS","probs_FS","probs_NS","PredictedSmokingStatus2","SmokingStatus"))
  message("============================================")
...
getLogOdds<- function( dataset, samplesheet, coefs)
{
  ...
  dataset_MLM  <- as.matrix( t(dataset_MLM ))
