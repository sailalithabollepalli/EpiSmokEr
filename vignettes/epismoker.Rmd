...
After installing **EpiSmokEr** load the packages along with its dependencies:
```{r}
...
In the example below result object from the **all** option of **epismoker** function has been saved to **result** variable, which will be passed onto generateReport function. On executing **generateReport** function, a **Results** folder will be created in the current working directory, with a html file named "Results_comprehensive.html". Result files generated from the example below can be found under the **Results** folder.
