...
- **Multinomial LASSO method (MLM)** provides smoking probabilities for each individual and **predicts smoking status**. MLM uses weights of 121 CpGs selected by multinomial LASSO approach. In addition to these 121 CpGs, sex and intercept terms are also used in the calculation.
These coefficients can be viewed using:
```{r}
read.table(file = 'inst/extdata/MLM_coefficients.tsv', sep = '\t', header = TRUE)
```
...
# Generate Results
***
Results can be saved as html and csv files using **generateReport** function. A **Results** folder will be created in the working directory. 
**generateReport** takes result object and method used in epismoker function. Users have to provide a name for the html file. 

#### Please note that result object from epismoker function must be saved as **result**.
In the example below result object from the **all** option of **epismoker** function has been saved to **result** variable, which will be passed onto generateReport function.

```{r}
generateReport(resObj = result, outputFileName = "Result", method = "all")
```
